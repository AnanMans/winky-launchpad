    ...
    <main className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      <div className="rounded-2xl border p-4 space-y-2">
        <div>
          ...
          {['Low', 'Medium', 'High'][coin.strength - 1]}
        </div>
      ...
      {/* Simple buy actions (dev first buy, etc.) */}
      <div className="rounded-2xl border p-4 space-y-2">
        <div className="font-medium">Buy (demo transfer)</div>
        <div className="flex gap-2">
          {[0.01, 0.05, 0.1].map((amt) => (
            ...
            >
              {buyLoading ? 'Sending…' : `Buy ${amt} SOL`}
            </button>
          ))}
        </div>
        {buySig && (
          <div className="text-xs opacity-70">
            Last signature: {buySig}
          </div>
        )}
      </div>
    </main>
